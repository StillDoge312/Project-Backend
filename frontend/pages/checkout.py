...
def _subscriptions_url(user_id: Optional[str]) -> str:
    if user_id:
        return f"/subscriptions?user_id={user_id}"
        ...
        with ui.column().classes("max-w-5xl mx-auto py-16 gap-10"):
            ...
            if plan:
                with ui.card().classes(
                    "mx-auto w-full max-w-xl p-6 bg-white shadow-md border border-gray-200 rounded-2xl"
                ):
                    ui.label(plan_title).classes("text-2xl font-semibold text-gray-900")
                    ui.label(
                        "Покажите инвесторам, как клиент проходит путь оплаты — никаких сторонних API не требуется."
            ...
            with ui.card().classes(
                ...
                def submit_payment():
                    ...
                    ui.navigate.to(_dashboard_url(user_id))
            ...
            ui.button(
                ...
                on_click=lambda: ui.navigate.to(_subscriptions_url(user_id)),
